(() => {
          ...
          (function(modules) {
            ...
            (function(module2, exports2, __webpack_require__) {
              ...
              function decodeKanji(stream, size) {
                ...
                var length = stream.readBits(characterCountSize);
                for (var i2 = 0; i2 < length; i2++) {
                  var k3 = stream.readBits(13);
                  var c2 = Math.floor(k3 / 192) << 8 | k3 % 192;
                  if (c2 < 7936) {
                    c2 += 33088;
                  } else {
                    c2 += 49472;
  ...
  function isDomainBlacklisted(url, blacklist = []) {
    ...
    try {
      ...
      return blacklist.some((item) => {
        const b2 = item.toLowerCase().trim();
        return hostname === b2 || hostname.endsWith(`.${b2}`);
      });
    } catch {
      return false;
    }
  ...
  function maskQrRegionInBuffer(imageData, location, margin = 4) {
    ...
    let maxY = Math.min(h2 - 1, Math.ceil(Math.max(pts[0].y, pts[1].y, pts[2].y, pts[3].y)));
    const n2 = pts.length;
    for (let y2 = minY; y2 <= maxY; y2++) {
      ...
      for (let i2 = 0; i2 < n2; i2++) {
        ...
        if (a2.y <= y2 && b2.y > y2 || b2.y <= y2 && a2.y > y2) {
          const t2 = (y2 - a2.y) / (b2.y - a2.y);
          xIntersections.push(a2.x + t2 * (b2.x - a2.x));
        }
      }
      if (xIntersections.length < 2) continue;
      xIntersections.sort((a2, b2) => a2 - b2);
      const xStart = Math.max(0, Math.floor(xIntersections[0]));
      const xEnd = Math.min(w3 - 1, Math.ceil(xIntersections[xIntersections.length - 1]));
      const rowBase = y2 * w3 * 4;
      for (let x2 = xStart; x2 <= xEnd; x2++) {
        const idx = rowBase + x2 * 4;
        data[idx] = 255;
        data[idx + 1] = 255;
        data[idx + 2] = 255;
        data[idx + 3] = 255;
  ...
  function computeFrameHash(dataUrl) {
    ...
    let hash = 2166136261;
    for (let i2 = 0; i2 < len; i2 += step) {
      hash ^= dataUrl.charCodeAt(i2);
  ...
  async function decodeDataUrl(dataUrl, maxW = 720, videoInfo = null) {
    ...
    try {
      ...
      const qrs = await decodeWithWorker(imgData.data.buffer, w3, h2);
      return { qrs, qr: qrs[0] || null, scanWidth: w3, scanHeight: h2 };
    } finally {
      bitmap.close();
    }
  }
