...
__export(overlay_exports, {
  QROverlayManager: () => QROverlayManager
});
function escapeHtml(str) {
  if (!str) return "";
  return String(str).replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;");
}
var QRBoxTracker, QROverlayManager;
...
function setupInPageSimulation() {
  ...
  simBtn.addEventListener("click", async () => {
    ...
    const { QROverlayManager: QROverlayManager2 } = await Promise.resolve().then(() => (init_overlay(), overlay_exports));
    const overlay = new QROverlayManager2({
      soundEnabled: true,
      autoCopy: false,
      onStopRequested: () => {
        overlay.unmount();
